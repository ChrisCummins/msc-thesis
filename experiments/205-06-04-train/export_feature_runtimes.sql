 ...
 * Export CSV datasets.
 *
 * features_runtimes.csv - Rows of (features, wgsize, runtime) observations.
 *   Multiple samples per feature vector and wgsize.
 * features_mean_runtimes.csv - Collated (features, wgsize, arithmetic mean
 *   runtime) rows. One row per combination of features and wgsize.
 * features_oracle_params.csv - Rows of (features, wgsize, mean runtime), where
 *   wgsize is the value which provided the lowest mean runtime. One row per
 *   feature vector.
 *
 * Usage:
 *     sqlite3 <db> < export_feature_runtimes.sql
 */

.mode csv
.header on

.once features_runtimes.csv
...
from (
    select * from (
        select * from (
            ...
            left join scenarios on runtimes.scenario=scenarios.id
...
.once features_mean_runtimes.csv
...
.once features_oracle_params.csv
