...
int main()
{
   ...
   size_t lowerBounds[] = {10};
   size_t upperBounds[] = {50000};
    ...
    */
//    int memFlags[] = {0}; /*! 0 means that that operand resides on main memory, 1 for gpu memory... */
//    skepu::ExecPlan execPlan1 = skepu::Tuner<plus_f, MAPREDUCE, plus_f>("globalSum", 1, lowerBounds, upperBounds, memFlags)();
//    globalSum.setExecPlan(execPlan1);
   
   skepu::ExecPlan execPlan[MAX_EXEC_PLANS];
   skepu::Tuner<plus_f, MAPREDUCE, plus_f> tuner("globalSum", 1, lowerBounds, upperBounds);
   tuner(execPlan);
   globalSum.setExecPlan(execPlan);
...
//       result = globalSum.CPU(v0);
      
      /// execution on OpenMP
//       result = globalSum.OMP(v0);
      
      /// execution on CPU
//       result = globalSum.CU(v0);
