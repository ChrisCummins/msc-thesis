...
int main()
{
   ...
   size_t lowerBounds[] = {10, 10, 10};
   size_t upperBounds[] = {50000, 50000, 50000};
    ...
    */
//    int memReadOpLoc[] = {0, 0}; /*! 0 means that the operand that needs to read resides on main memory, 1 for gpu memory... */
...
//    skepu::ExecPlan execPlan1 = skepu::Tuner<arr_f, MAPARRAY, arr_f>("reverseArray", 3, lowerBounds, upperBounds, memReadOpLoc, memWriteOpLoc)();
//    reverse.setExecPlan(execPlan1);
   
   skepu::ExecPlan execPlan[MAX_EXEC_PLANS];
   skepu::Tuner<arr_f, MAPARRAY, arr_f> tuner("reverseArray", 3, lowerBounds, upperBounds);
   tuner(execPlan);
   reverse.setExecPlan(execPlan);
...
//       reverse.CPU(v0, v1, v2);
      
      /// execution on OpenMP
//       reverse.OMP(v0, v1, v2);
...
//       reverse.CU(v0, v1, v2);
      ...
      std::cerr << "Size: " << size << ", Result: " << v2[0] << "\n";
   }
   return 0;
}
