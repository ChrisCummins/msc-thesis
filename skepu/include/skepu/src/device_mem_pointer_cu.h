...
/*! specified the maximum size for total sub copies needed to fill a copy data... */
#define MAX_COPYINF_SIZE 10
...
// #endif   
   ...
   void copyAllRangesToDevice(UpdateInf<T>* updateStruct, const size_t sizeUpdStr);
...
DeviceMemPointer_CU<T>::DeviceMemPointer_CU(T* start, size_t numElements, Device_CU *device, std::string name) : m_hostDataPointer(start), m_numElements(numElements), m_dev(device), m_rows(1), m_cols(numElements), m_pitch(numElements), m_valid(false), m_usePitch(false), m_numOfRanges(0)
{
   ...
   CHECK_CUDA_ERROR(cudaMalloc((void**)&m_deviceDataPointer, sizeVec));
 ...
 */
template <typename T>
void DeviceMemPointer_CU<T>::copyAllRangesToDevice(UpdateInf<T>* updateStruct, const size_t sizeUpdStr)
{
   ...
   for(int i=0; i<sizeUpdStr; ++i)
   {
      ...
      if(m_usePitch)
         ...
         CHECK_CUDA_ERROR(cudaMemcpyAsync(m_deviceDataPointer + updateStruct[i].dstOffset, updateStruct[i].src + updateStruct[i].srcOffset, sizeVec, memKind, (m_dev->m_streams[0])))
      else
