...
/*!
 *  Performs the MapOverlap on a whole Vector using \em OpenMP as backend and itself as output. A wrapper for
 *  OMP(InputIterator inputBegin, InputIterator inputEnd, EdgePolicy poly, typename InputIterator::value_type pad).
 *
...
template <typename T>
void MapOverlap<MapOverlapFunc>::OMP(Vector<T>& input, EdgePolicy poly, T pad)
{
   OMP(input.begin(), input.end(), poly, pad);
}

/*!
 *  Performs the MapOverlap on a range of elements using \em OpenMP as backend and the same range as output. Since a seperate output is needed, a
...
void MapOverlap<MapOverlapFunc>::OMP(InputIterator inputBegin, InputIterator inputEnd, EdgePolicy poly, typename InputIterator::value_type pad)
{
   Vector<typename InputIterator::value_type> output;
   output.clear();
   output.resize(inputEnd-inputBegin);

   OMP(inputBegin, inputEnd, output.begin(), poly, pad);
...
/*!
 *  Performs the MapOverlap on a Matrix on the \em OpenMP with the same Matrix as output.
 *  Wrapper for  OMP(InputIterator inputBegin, InputIterator inputEnd, OverlapPolicy overlapPolicy,
...
template <typename MapOverlapFunc>
template <typename T>
void MapOverlap<MapOverlapFunc>::OMP(Matrix<T>& input, OverlapPolicy overlapPolicy, EdgePolicy poly, T pad)
{
   OMP(input.begin(), input.end(), overlapPolicy, poly, pad);
}

/*!
 *  Performs the MapOverlap on a range of elements on the \em OpenMP with the
 *  same range as output. Since a seperate output is needed, a
...
template <typename MapOverlapFunc>
template <typename InputIterator>
void MapOverlap<MapOverlapFunc>::OMP(InputIterator inputBegin, InputIterator inputEnd, OverlapPolicy overlapPolicy, EdgePolicy poly, typename InputIterator::value_type pad)
{
   ...
   OMP(inputBegin, inputEnd, output.begin(), overlapPolicy, poly, pad);
...
/*!
 *  Performs the MapOverlap on a whole Vector using \em OpenMP as backend and a seperate output vector. Wrapper for
 *  OMP(InputIterator inputBegin, InputIterator inputEnd, OutputIterator outputBegin, EdgePolicy poly, typename InputIterator::value_type pad).
 *
...
template <typename MapOverlapFunc>
template <typename T>
void MapOverlap<MapOverlapFunc>::OMP(Vector<T>& input, Vector<T>& output, EdgePolicy poly, T pad)
{
   ...
   OMP(input.begin(), input.end(), output.begin(), poly, pad);
}
...
void MapOverlap<MapOverlapFunc>::OMP(InputIterator inputBegin, InputIterator inputEnd, OutputIterator outputBegin, EdgePolicy poly, typename InputIterator::value_type pad)
{
   DEBUG_TEXT_LEVEL1("MAPOVERLAP OpenMP\n")
   ...
   //Copy values
   size_t j = 0;
   for(size_t i = overlap; i < 3*overlap; ++i)
   {
...
/*!
 *  Performs the MapOverlap on a whole Matrix on the \em OpenMP with a seperate output Matrix. Wrapper for
 *  OMP(InputIterator inputBegin, InputIterator inputEnd, OutputIterator outputBegin, OverlapPolicy overlapPolicy, EdgePolicy poly, typename InputIterator::value_type pad).
 *
...
template <typename MapOverlapFunc>
template <typename T>
void MapOverlap<MapOverlapFunc>::OMP(Matrix<T>& input, Matrix<T>& output, OverlapPolicy overlapPolicy, EdgePolicy poly, T pad)
{
   if(input.size() != output.size())
   {
      output.clear();
   ...
   OMP(input.begin(), input.end(), output.begin(), overlapPolicy, poly, pad);
}

/*!
 *  Performs the MapOverlap on a range of elements on the \em OpenMP with a seperate output range.
 *
 *  \param inputBegin A Matrix::iterator to the first element in the range.
 *  \param inputEnd A Matrix::iterator to the last element of the range.
...
template <typename MapOverlapFunc>
template <typename InputIterator, typename OutputIterator>
void MapOverlap<MapOverlapFunc>::OMP(InputIterator inputBegin, InputIterator inputEnd, OutputIterator outputBegin, OverlapPolicy overlapPolicy, EdgePolicy poly, typename InputIterator::value_type pad)
{
   DEBUG_TEXT_LEVEL1("MAPOVERLAP OpenMP\n")
   ...
   if(overlapPolicy == OVERLAP_ROW_COL_WISE)
   {
      ...
      OMP_ROWWISE(inputBegin,inputEnd,tmp_m.begin(), poly, pad);
      OMP_COLWISE(tmp_m.begin(),tmp_m.end(),outputBegin, poly, pad);
   }
   else if(overlapPolicy == OVERLAP_COL_WISE)
      OMP_COLWISE(inputBegin,inputEnd,outputBegin, poly, pad);
   else
      OMP_ROWWISE(inputBegin,inputEnd,outputBegin, poly, pad);
}
...
template <typename MapOverlapFunc>
template <typename InputIterator, typename OutputIterator>
void MapOverlap<MapOverlapFunc>::OMP_ROWWISE(InputIterator inputBegin, InputIterator inputEnd, OutputIterator outputBegin, EdgePolicy poly, typename InputIterator::value_type pad)
{
   size_t totElem= inputEnd - inputBegin;
   ...
   for(size_t row=0; row< (totElem/rowWidth); row++)
   {
...
template <typename MapOverlapFunc>
template <typename InputIterator, typename OutputIterator>
void MapOverlap<MapOverlapFunc>::OMP_COLWISE(InputIterator inputBegin, InputIterator inputEnd, OutputIterator outputBegin, EdgePolicy poly, typename InputIterator::value_type pad)
{
   size_t n= inputEnd - inputBegin;
