...
SparseMatrix<T>::SparseMatrix(size_t rows, size_t cols, size_t nnz, T *values, size_t *rowPtr, size_t *colInd, bool dealloc, T zeroValue, bool transMatrix): m_rows(rows), m_cols(cols), m_nnz(nnz), m_values(NULL), m_rowPtr(NULL), m_colInd(NULL), m_dealloc(dealloc), m_zeroValue(zeroValue), m_transposeValid(false), m_colPtrCSC(NULL), m_rowIndCSC(NULL), m_valuesCSC(NULL), m_cscMatrix(NULL), m_transMatrix(transMatrix)
{
   ...
   if(!m_transMatrix) // if not a transposed matrix then perform this check.
   {
      ...
      allocateHostMemory<T>(m_values, m_nnz); // can be pinned if enabled
      std::copy(&values[0], &values[m_nnz], m_values);

      allocateHostMemory<size_t>(m_rowPtr, m_rows+1);
      std::copy(&(rowPtr[0]), &(rowPtr[m_rows]), m_rowPtr);

      allocateHostMemory<size_t>(m_colInd, m_nnz);
...
SparseMatrix<T>::SparseMatrix(size_t rows, size_t cols, size_t nnz, T min, T max, T zeroValue): m_rows(rows), m_cols(cols), m_nnz(nnz), m_values(NULL), m_rowPtr(NULL), m_colInd(NULL), m_dealloc(true), m_zeroValue(zeroValue), m_transposeValid(false), m_colPtrCSC(NULL), m_rowIndCSC(NULL), m_valuesCSC(NULL), m_cscMatrix(NULL), m_transMatrix(false)
{
   ...
   for(; i<m_rows && k<nnz; i++)
   {
      ...
      while(j<totalElems)
      {
         size_t colIdx= get_random_number((size_t)0, m_cols-1);
      ...
      {
         m_colInd[k] = *colIter;
         m_values[k] = get_random_number(min, max);
         k++;
      }
...
SparseMatrix<T>::SparseMatrix(const SparseMatrix<T> &copy): m_rows(copy.m_rows), m_cols(copy.m_cols), m_nnz(copy.m_nnz), m_dealloc(true), m_zeroValue(copy.m_zeroValue), m_transposeValid(false), m_colPtrCSC(NULL), m_rowIndCSC(NULL), m_valuesCSC(NULL), m_cscMatrix(NULL), m_transMatrix(false)
{
   ...
   allocateHostMemory<size_t>(m_colInd, m_nnz); // can be pinned if enabled
   allocateHostMemory<size_t>(m_rowPtr, m_rows+1); // can be pinned if enabled

   if(copy.m_values)
      std::copy(&(copy.m_values[0]), &(copy.m_values[m_nnz]), m_values);

   if(copy.m_rowPtr)
      std::copy(&(copy.m_rowPtr[0]), &(copy.m_rowPtr[m_rows]), m_rowPtr);

   if(copy.m_colInd)
      std::copy(&(copy.m_colInd[0]), &(copy.m_colInd[m_nnz]), m_colInd);
}
...
void SparseMatrix<T>::operator=(const SparseMatrix<T> &other)
{
   ...
   {
      if(m_values)
         deallocateHostMemory<T>(m_values);
      
      if(m_colInd)
         deallocateHostMemory<T>(m_colInd);

      m_nnz = other.m_nnz;

      allocateHostMemory<T>(m_values, m_nnz);
      allocateHostMemory<size_t>(m_colInd, m_nnz);
   }
   if(m_rows!=other.m_rows)
   {
      if(m_rowPtr)
         deallocateHostMemory<T>(m_rowPtr);
      
      m_rows = other.m_rows;

      allocateHostMemory<T>(m_rowPtr, m_rows+1);
   }
   m_cols = other.m_cols;
   m_dealloc = true;
...
SparseMatrix<T>::~SparseMatrix()
{
   ...
   if(!m_transMatrix)
   {
      if(m_dealloc)
      {
         ...
         if(m_rowPtr!=NULL)
            deallocateHostMemory<size_t>(m_rowPtr);
         if(m_colInd!=NULL)
            deallocateHostMemory<size_t>(m_colInd);
      }
...
void SparseMatrix<T>::resize(SparseMatrix<T> &copy, bool retainData)
{
   ...
   if(m_nnz != copy.m_nnz)
   {
      ...
      m_values = allocateHostMemory<T>(copy.m_nnz);
...
// ***** allocate storage
   ...
   if(m_values==NULL)
   {
      ...
      m_values = allocateHostMemory<T>(m_nnz); // can be pinned if enabled
   }
 ...
 */
template <typename T>
inline void SparseMatrix<T>::updateHost() const
{
#ifdef SKEPU_OPENCL
   updateHost_CL();
#endif
 ...
 */
template <typename T>
inline void SparseMatrix<T>::invalidateDevice()
{
#ifdef SKEPU_OPENCL
