 ...
 */
template <typename T>
void Matrix<T>::copyDataToAnInvalidDeviceCopy(DeviceMemPointer_CU<T> *copy, unsigned int deviceID)
{
   ...
   UpdateInf<T> updateStruct[MAX_COPYINF_SIZE];
...
//                      if(it->second->deviceDataHasChanged())
   ...
   if(copy->m_numOfRanges != 0)
   {
      ...
      {
         Environment<int> *env = Environment<int>::getInstance();
...
// out_label_2: /*! now copy from host memory... the m_valid flag could be false at this time... updates later in function... */
   ...
   */
   copy->copyAllRangesToDevice(updateStruct, sizeUpdStr);
...
*/
template <typename T>
typename Matrix<T>::device_pointer_type_cu Matrix<T>::updateDevice_CU(T* start, size_type rows, size_type cols, unsigned int deviceID, bool copy, bool writeAccess, bool usePitch, bool markOnlyLocalCopiesInvalid)
{
...
//    assert(usePitch == false); // TODO: fix this...
   ...
   {
      tempCopy = new DeviceMemPointer_CU<T>(start, rows, cols, Environment<int>::getInstance()->m_devices_CU.at(deviceID), usePitch, m_nameVerbose);
      if(copy)
      {
   ...
   if(result == m_deviceMemPointers_CU[deviceID].end()) //insert new, alloc mem and copy
   {
      ...
      if(copy)
      {
         copyDataToAnInvalidDeviceCopy(tempCopy, deviceID);
      }
      result = m_deviceMemPointers_CU[deviceID].insert(m_deviceMemPointers_CU[deviceID].begin(), std::make_pair(key,tempCopy));
   }
   else
...
*/
template <typename T>
typename Matrix<T>::device_pointer_type_cu Matrix<T>::updateDevice_CU(T* start, size_type cols, unsigned int deviceID, bool copy, bool writeAccess, bool markOnlyLocalCopiesInvalid)
{
   return updateDevice_CU(start, (size_type)1, cols, deviceID, copy, writeAccess,markOnlyLocalCopiesInvalid);
}
...
inline void Matrix<T>::updateHost_CU(int deviceID) const
{
   ...
   if(deviceID < 0) // do it for all devices....
   {
      for(deviceID = 0; deviceID < MAX_GPU_DEVICES; ++deviceID)
      {
         ...
         Environment<int>::getInstance()->finishAll_CU(deviceID, deviceID+1);
      }
   }
...
*/
template <typename T>
inline void Matrix<T>::invalidateDeviceData_CU(int deviceID)
{
...
*/
template <typename T>
bool Matrix<T>::isModified_CU(unsigned int deviceID)
{
   typename std::map< std::pair<T*,size_type>, device_pointer_type_cu >::iterator result;
...
*/
template <typename T>
inline void Matrix<T>::releaseDeviceAllocations_CU(int deviceID)
{
