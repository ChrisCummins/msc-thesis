...
/*!
 *  Performs the 2D MapOverlap on a Matrix on the \em CPU with the same Matrix as output.
...
template <typename T>
void MapOverlap2D<MapOverlap2DFunc>::CPU(Matrix<T>& input)
{
...
/*!
 *  Performs the 2D MapOverlap on a whole matrix on the \em CPU with a separate output matrix.
 *  The actual filter is specified in a user-function.
 *
...
template <typename MapOverlap2DFunc>
template <typename T>
void MapOverlap2D<MapOverlap2DFunc>::CPU(Matrix<T>& input, Matrix<T>& output)
{
   DEBUG_TEXT_LEVEL1("2D MAPOVERLAP CPU\n")
...
/*!
 *  Performs the 2D MapOverlap on the \em CPU, based on provided filter and input neighbouring elements on a whole Matrix.
...
template <typename MapOverlap2DFunc>
template <typename T>
void MapOverlap2D<MapOverlap2DFunc>::CPU(Matrix<T>& input, Matrix<T>& output, Matrix<T>& filter)
{
   DEBUG_TEXT_LEVEL1("2D MAPOVERLAP with Filter Matrix CPU\n")
...
/*!
 *  Performs the 2D MapOverlap on the \em CPU, by taking average of neighbouring elements on a whole Matrix.
 *  With a separate Matrix as output.
 *
 *  \param input A matrix which the mapping will be performed on. It should include padded data as well considering the filter size
 *  \param output The result matrix, will be overwritten with the result and resized if needed.
...
template <typename MapOverlap2DFunc>
template <typename T>
void MapOverlap2D<MapOverlap2DFunc>::CPU(Matrix<T>& input, Matrix<T>& output, size_t filter_rows, size_t filter_cols)
{
   DEBUG_TEXT_LEVEL1("2D MAPOVERLAP with Average Filter CPU\n")
