...
timers have been added which report millisecond execution time of
three of the

\begin{itemize}
\item \textbf{init} All programs must initialise SkelCL by calling
  \texttt{skelcl::init()}, passing as parameters the type and count of
  execution device. A list of OpenCL platforms and devices are then
  enumerated, and any devices matching the specified type and count

\item \textbf{build}
Per skeleton.
Compiled programs are cached for re-use.
\item \textbf{prep}
e.g. allocating device buffers for output data.
\end{itemize}
...
\begin{itemize}
\item \textbf{upload} Host $\rightarrow$ Device memory transfer.
\item \textbf{run}
\item \textbf{download} Device $\rightarrow$ Host memory transfer.
\end{itemize}
...
increasing number of iterations. The ``Break-even point'' is the
number of iterations at which point the Stencil kernel is faster than
MapOverlap.
\begin{table}
\footnotesize
\centering
