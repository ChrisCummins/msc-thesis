...
\caption{%
  Number of sample counts per instance.%
}
...
average of \input{gen/avg_sample_count} runtimes test case (min
\input{gen/min_sample_count}). Figure~\ref{fig:sample-counts} shows
plots the ratio of test cases as a function of the minimum number of
samples. Figure~\ref{fig:min-max-runtimes} shows the distribution of
minimum and maximum runtimes for each combination of scenario and
workgroup size.
