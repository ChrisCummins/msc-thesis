...
size. This demonstrates that tuning for optimal workgroup size is a
non-trivial task, requiring 14 distinct workgroup sizes to achieve
50\% oracle accuracy (Figure~\ref{fig:oracle-accuracy}).
...
the highest mean performance are legal for less than 1\% of all
scenarios, which reinforces the claim for autotuning. The workgroup
sizes with the highest legality are listed in
...
we find the average of performance upper bounds to be $15.14\times$
(min $1.03\times$, max $207.72\times$). This demonstrates the
...
$207.72\times$. Note too that for 5 of the scenarios, the speedup of
the best over worst workgroup sizes is $\le 5\%$.
...
\subsection{Evaluting Regressors}
...
\rowcolors{2}{white}{gray!25}
\input{gen/tab/class}
\caption{Validation results for J48 and \textsc{NearestNeighbour} classification.}
\end{table}
\begin{table}
\scriptsize
\centering
