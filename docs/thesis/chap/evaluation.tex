...
skeleton~\cite{Breuer2013}, \citeauthor{Beers2013} selected a
...
%
\begin{equation}
  D_{training} = \cup \left\{ f(s),t(s,w) | w \in W_{legal}(s)
...
using regressors always results in a legal prediction, so no fallback
...
Figures~\ref{fig:class-syn} and~\ref{fig:class-arch} show
classification results for 10-fold cross-validation and cross-device
...
\caption[Stencil speedups using classification]{%
...
\rowcolors{2}{white}{gray!25}
\input{gen/tab/class}
\caption{J48 and \textsc{NearestNeighbour} classification}
\end{table}
\begin{table}
\scriptsize
\centering
...
\caption{Speedup regression}
\end{table}


Much greater variance in prediction times than with the classifiers,
