...
workgroup sizes can be shown to be non-portable across architectures,
kernels, and datasets, as shown in Figure~\ref{fig:performances}.
...
\caption{%
  ...
  of: (\ref{fig:performance-max-wgsize})~maximum legal size,
  (\ref{fig:performance-wg-c})~number of columns, and
  (\ref{fig:performance-wg-r})~ number of rows. As can be seen,
...
\begin{figure}
\include{fig/performances}
\caption{%
  Performance relative to the oracle of workgroup sizes across all
  scenarios, grouped by: (\ref{fig:performance-kernels})~kernels,
  (\ref{fig:performance-devices})~devices, and
  (\ref{fig:performance-datasets})~datasets.%
}
