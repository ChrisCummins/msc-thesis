...
\input{gen/num_params} possible combinations of workgroup
size. \TODO{Justify this choice of parameter values.}
...
The geometric mean is used to calculate aggregate means of relative
performances, due to it's multiplicative property~\cite{Fleming1986}.
...
From the set of all scenarios $S$ and workgroup sizes $W$, the subset
of workgroup scenarios $W_{legal}(S) \subset W$ which are legal for a
given scenario can be found using:
