...
and autotuner for 3D Jacobi stencil codes on GPUs. Using a DSL for
expressing kernel functions, the code generator performs substitution
from one of two CUDA templates to creates programs. These programs are
parameterised by block size, block dimensions, and whether input data
is stored in read only texture memory. This provides a search space of
up to 200 configurations, which is exhaustively enumerated so as to
