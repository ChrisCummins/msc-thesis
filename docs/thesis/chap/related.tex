...
which maximises performance. The first formalised publication of the
technique appeared in \citeyear{Bodin1998} by
\citeauthor{Bodin1998}~\cite{Bodin1998}.  Iterative compilation has
been demonstrated to be a highly effective form of empirical
...
heuristics provides the greatest level of generalisation, but is an
ongoing research challenge~\cite{Burke2013}.
...
present from querying a single centralised data store during a
program's runtime. The current implementation of Collective Mind uses
a NoSQL JSON format for storing performance data. The relational
schema used in this thesis offers greater scaling performance as the
amount of performance data from collective tuning grows.
...
tool which generates randomised C source code for automatically
detecting compiler bugs~\cite{Yang2012}.
...
over a user-defined range of values to generate programs training
programs with a statistically controlled range of features. A Perl
preprocessor generates output source codes from an input description
using a custom input language Genesis. Genesis is more flexible than
the system presented in this thesis, supporting substitution of
arbitrary sources. The authors describe an application of their tool
for generating OpenCL stencil kernels, but do not report any
performance results.
